...
\comment{Preveril sem, da je porazdelitev vseh parametrov enakomerna čez cel dataset.}

\comment{Popravi tabelo, da pride lepo na eno stran!}
...
\begin{figure}[ht]
	\includegraphics[width=15.6cm]{exploratory/ea_corr_data.pdf}
	\caption{Koreliranost značilk v simuliranih (merjenih) podatkih.}
	\label{corr_matrix}
\end{figure}
\comment{Slika \ref{corr_matrix} ne pove nič več kot \ref{corr_clust_matrix}, zato lahko gre ven.}

\begin{figure}[ht]
	\includegraphics[width=15.6cm]{exploratory/ea_corr_clustermap.pdf}
...
\comment{Ne smemo ravno povprečiti ...}
...
\section{PCA in LDA}
\comment{Naredil sem PCA - obstaja tabela vrednosti; vrednotenje.}
\comment{Naredil sem vizualizacijo v 2D; z vsemi sampli obeta, da se nekako da zaznati}
\comment{regijo, kjer je več dogodkov, pri samplanju na 2000-10000 eventov pa tega ni videt;}
\comment{mogoče je to samo artefakt vizualizacije.}

\comment{Mogoče naredim še kakšno LDA vizualizacijo.}
\comment{Manjka nekaj zgodbe iz te analize!}