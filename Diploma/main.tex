...
\noindent Diplomsko delo obravnava problem ločevanja signala in ozadja v eksperimentu ATLAS pri razpadu $H \rightarrow \tau^+\tau^-$. Reševanje problema temelji na skupku algoritmov, ki jih v računalništvo imenujemo tudi \textit{strojno učenje}. Algoritmi so uspešni pri iskanju področij prostora meritev, ki ustrezajo signalu.

Zaradi podobnosti drugim razpadom, je razpad $H \rightarrow \tau^+\tau^-$ še posebej težaven za klasifikacijo. Diplomsko delo se naslanja na podatke in nekatere izsledke iz javnega izziva HiggsML, ki so ga z namenom iskanja najboljših algoritmov za klasifikacijo razpadov, ki ustrezajo $H \rightarrow \tau^+\tau^-$, organizirali sodelavci kolaboracije ATLAS.

V diplomskem delu je predstavljen kratek opis problema. Izbrane relevantne metode strojnega učenja, ki so jih identificirali organizatorji izziva, so predstavljene in preizkušene na podatkih simulatorja detektorja ATLAS. Poleg omenjenih metod sem v diplomskem delu razvil lasten algoritem, ki temelji na metodi podpornih vektorjev.

Uspešnost algoritma presega nekatere implementacije metode pospešenega gradientnega spusta na odločitvenih drevesih, ki jo trenutno uporabljajo pri klasifikaciji dogodkov na detektorju ATLAS, vseeno pa krepko zaostaja za zmagovalnimi rešitvami.
...
\vspace{0.5cm}
\noindent strojno učenje, nadzorovano učenje, klasfikacijske metode, pospešena odločitvena drevesa, metoda podpornih vektorjev, SVM, Higgsov bozon
		...
		\Fn{\FRecurs{${\cal D}$, ${\cal A}$}}{								
			...
			\Begin{
				...
				\For{vse možne vrednosti $v_i$ značilke $Z$\;} {
					...
					naj bo ${\cal D}_{v_i}$ podmnožica ${\cal D}$ vrednostjo $Z = v_i$\;
					\uIf{${\cal D}_{v_i} = \{\}\;$} {
...
Pregled povzetka značilk nam pove, da so značilke, ki se med razredi najbolj razlikujejo (npr. DER\_mass\_MMC, DER\_mass\_transverse\_met\_lep, DER\_deltaeta\_jet\_jet, PRI\_jet\_leading\_eta). Povzetek vseh značilk je prikazan v tabeli \ref{tb:5figuresum}. Natančnejši pogled posamičnih značilk je možen s pomočjo histogramov. Nekaj predstavnikov, pri katerih se porazdelitve med razredi najbolj razlikujejo, je predstaviljenih na sliki \ref{sl:histogrami}, celoten pregled izmerjenih in izračunanih značilk pa je na voljo na slikah \ref{sl:histogram_izmerjene} in \ref{sl:histogram_izpeljane}.
...
Ideja z vključevanjem zaporedne številke grozdov iz algoritma KMeans ni prinesla posebnega uspeha. 
...
Preskusil sem naslednja standardna jedra SVM: (1) linearnega, (2) polinomskega in (3) RBF (Gaussovskega). Rezultati so zabeleženi v tabeli \ref{tb:svm_kernel},
...
Uporabil sem ansambelski metodi zlaganja in kaskade. Pri zlaganju sem vsak vzorec iz testne podatkovne množice ovrednotil z vsakim od 9 šibkih algoritmov, končni rezultat pa podal na podlagi glasovanja. Rezultati so prikazani v tabeli \ref{tb:svm-ensemble}. Rezultat metode je v konkretnem malenkost boljši od posameznih šibkih metod, hkrati pa opazno slabši od SVM metode, pognane na celotni razpoložljivi učni množici podatkov. Zlaganje torej ni prineslo izboljšanja.

Pri kaskadi sem uporabil 8 šibkih algoritmov, kjer sem algoritem SVM pognal na podlagi njihovih rezultatov (kot značilkami) pri drugostopenjski SVM klasifikaciji na 9. delu učne podatkovne množice. Rezultati te metode so bili še slabši od zlaganja.
...
\begin{table}[h!]
	...
	zlaganje (glasovanje) & 24,4\% & 58,7\% & 6,9\% & 10,0\% &
		...
		2,854 \\
	kaskada (SVM) & 24,4\% & 58,5\% & 7,0\% & 10,0\% &
